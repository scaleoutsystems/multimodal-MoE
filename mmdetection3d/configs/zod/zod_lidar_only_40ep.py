...
# ---------------------------------------------------------------------------
# LiDAR-only BEVFusion / ZOD — 40-epoch run
# ---------------------------------------------------------------------------
# Identical to zod_lidar_only_30ep.py (run 4570893) except:
#   - max_epochs 30 → 40
#   - decay phase extended from 8–30 (T_max=22) to 8–40 (T_max=32)
#   - warmup window 0→8 unchanged
#
# Motivation: run 4570893 peaked at epoch 27 (val AP@0.50=0.5425), still
# improving at epoch 30.  40 epochs provides additional headroom for
# convergence and forms a fair iso-budget comparison with the 40-epoch
# MoE runs.
# ---------------------------------------------------------------------------
...
# 40-epoch cosine schedule — warmup 0→8 unchanged, decay extended 8→40.
# T_max for decay phase = 40 - 8 = 32.
lr = 5e-5
param_scheduler = [
    dict(
