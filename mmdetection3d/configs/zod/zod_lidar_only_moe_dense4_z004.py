...
bev_moe_cfg = dict(
    ...
    # ST-MoE router z-loss — anchors the logit scale.  Increased to 4e-3
    # from 5e-4 (4563095) to prevent the clean-gate logit std from growing
    # unchecked.  In 4563095 val clean_logits_std grew from 0.59 → 1.50 over
    # 7 epochs while ctx_logits_mean_abs stayed flat at ~1.0, causing the
    # context/clean ratio to drop from 1.74 → 0.67 — context progressively
    # lost steering authority over routing.  Stronger z-loss targets
    # clean_logits_std ≤ ~1.0 to maintain ctx/clean ratio ≥ 1.0 throughout.
    z_loss_coef=4e-3,  # LAST RUN: 5e-4 (4563095), 1e-3 (4552697), 2e-4 (4554362)
    ...
    residual_gain=1.0,
    # Context-supervised routing — loss_coef raised from 0.03 → 0.05.
    # In 4563095 ctx_aux_acc oscillated 0.47–0.62 and never clearly
    # improved, partly because the context gradient budget (coef=0.03)
    # was too small to keep ctx_logits_mean_abs competitive as the clean
    # gate grew.  Raising to 0.05 modestly strengthens the context signal
    # without approaching the 0.10 regime that caused the epoch-8 collapse
    # in 4562168 (where ctx CE ~1.1 contributed ~8% of total loss).  At
    # coef=0.05 and the same CE level the context contribution stays ~4%
    # of total loss — well below the collapse threshold.
