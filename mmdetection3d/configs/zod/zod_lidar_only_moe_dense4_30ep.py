...
# 30-epoch extended run with z_loss_coef=2e-3
# ---------------------------------------------------------------------------
...
#
# Changes vs zod_lidar_only_moe_dense4_z004.py (4563904):
#
#   1. max_epochs 20 → 30 with scheduler extended accordingly.
#      Run 4563095 (dense4, z_loss=5e-4) showed a pronounced late-stage
#      rise: val AP@0.50 jumped +4.8 pp from epoch 16→19 (0.4643 → 0.5122)
#      and was still climbing.  The 20-epoch budget cut it short.  This run
#      extends to 30 epochs while keeping the same warmup window (0→8) and
#      extending only the cosine decay phase (8→30).
#
#   2. z_loss_coef 4e-3 → 2e-3.
#      4e-3 (z004 / 4563904) over-regularised the gate: best val AP@0.50
#      peaked at 0.4718 vs 0.5122 for 4563095 (z_loss=5e-4).  But 5e-4 let
#      clean_logits_std grow 0.59 → 1.50 over 7 epochs while
#      ctx_logits_mean_abs stayed ~1.0, pushing the ctx/clean ratio below
#      1.0 and degrading routing authority late in training.  2e-3 is a
#      middle position: strong enough to partially anchor the logit scale
#      (targeting std ≈ 1.0–1.2 rather than unconstrained growth) without
#      the heavy suppression that collapsed task performance in z004.
#
#   3. loss_coef unchanged at 0.03 (current source default).
#
# ---------------------------------------------------------------------------
...
bev_moe_cfg = dict(
    ...
    # ST-MoE router z-loss — set to 2e-3 as a middle position between
    # 5e-4 (4563095, unconstrained logit growth) and 4e-3 (z004/4563904,
    # over-regularised, best val AP@0.50 capped at 0.4718).  Targets
    # clean_logits_std ≈ 1.0–1.2 to maintain ctx/clean steering ratio
    # above 1.0 without the heavy task-performance suppression seen in z004.
    z_loss_coef=2e-3,  # HISTORY: 5e-4 (4563095), 4e-3 (z004/4563904)
    residual_gain=1.0,
...
# ── Schedule ──────────────────────────────────────────────────────────────
# 30-epoch cosine schedule.  Warmup window kept at 0→8 (identical to the
# 20-epoch runs) so the pretrained weights stabilise on the same budget.
# Decay phase extended from 8→20 (T_max=12) to 8→30 (T_max=22).
lr = 5e-5
param_scheduler = [
    dict(
    ...
    dict(
        type='CosineAnnealingLR',
        T_max=22, eta_min=lr * 1e-4,
        begin=8, end=30, by_epoch=True, convert_to_iter_based=True),
    dict(
