"""Camera + LiDAR BEVFusion — controlled dual-init baseline, 40-epoch schedule.

Derived from zod_bevfusion_dualinit.py (run 4543552, 12 epochs).  Only the
settings listed below differ from the 12-epoch dual-init config; everything
else (model architecture, data pipelines, dataloaders, evaluators, optimizer,
AMP wrapper) is kept identical.

Changes from zod_bevfusion_dualinit.py
---------------------------------------
  1. max_epochs 12 → 40
  2. Decay phase extended: CosineAnnealingLR phase 2 from 4→12 (T_max=8) to
     4→40 (T_max=36).  Momentum scheduler updated to match.
  3. New checkpoints from later unimodal runs:
       lidar_ckpt  ← best_mAP_0.50_epoch_30.pth  (zod-lidar-only_4570893, 0.544 mAP)
       camera_ckpt ← best_mAP_0.50_epoch_31.pth  (zod-cam-only_4577582)
  4. Checkpoint save interval 2 → 5 (appropriate for longer run).
  5. _VIS_EPOCHS updated for 40-epoch span.
  6. GridMask max_epoch 10 → 40.

Purpose
-------
Extended training budget (40 epochs) to match the 40-epoch LiDAR-only and
camera-only unimodal baselines, providing a fair iso-budget comparison.
Both branches are warm-started from the best available unimodal checkpoints
(epoch 34 LiDAR, epoch 31 camera) from the 40-epoch unimodal runs.

Training schedule
-----------------
  lr          = 5e-5
  warmup      = LinearLR(start_factor=1/3, 500 iters)
  cosine      = phase 1 (0→4 ep, eta_min=lr*10) + phase 2 (4→40 ep, eta_min=lr*1e-4)
...
# ===== optimizer / scheduler =====
# 40-epoch cosine schedule.
# Warmup (500 iters) and phase-1 cosine (0→4 ep) unchanged from 12-ep config.
# Decay phase extended: 4→12 (T_max=8) → 4→40 (T_max=36).
...
#   epoch 4 → 40   : cosine   lr → eta_min = lr * 1e-4 = 5e-9
...
_VIS_EPOCHS = (1, 5, 10, 15, 20, 25, 30, 35, 40)
