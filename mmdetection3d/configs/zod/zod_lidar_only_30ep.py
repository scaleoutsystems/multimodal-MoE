...
# ---------------------------------------------------------------------------
# LiDAR-only BEVFusion / ZOD — 30-epoch run (fair comparison baseline)
# ---------------------------------------------------------------------------
# Identical to zod_lidar_only.py (run 4543546) except:
#   - max_epochs 20 → 30
#   - scheduler decay phase extended 8→20 (T_max=12) to 8→30 (T_max=22)
#   - warmup window 0→8 unchanged
#
# Motivation: run 4543546 peaked at epoch 20 (best_epoch=20, val AP@0.50=0.5148)
# with the final epoch = best, meaning the model had not converged.  The paired
# MoE run (dense4_30ep) also extends to 30 epochs, so the same budget is needed
...
# 30-epoch cosine schedule — warmup 0→8 unchanged, decay extended 8→30.
lr = 5e-5
param_scheduler = [
    dict(
