...
# 40-epoch extended run with z_loss_coef=2e-3
# ---------------------------------------------------------------------------
# Identical to zod_lidar_only_moe_dense4_30ep.py (run 4570728) except:
#   - max_epochs 30 → 40
#   - decay phase extended from 8–30 (T_max=22) to 8–40 (T_max=32)
#   - warmup window 0→8 unchanged
#
# Motivation: run 4570728 (30 ep) to provide additional headroom for
# convergence and a fair iso-budget comparison with the 40-epoch
# lidar-only baseline and task-driven MoE run.
# ---------------------------------------------------------------------------
...
# 30-epoch run).  Decay phase extended from 8→30 (T_max=22) to 8→40 (T_max=32).
lr = 5e-5
param_scheduler = [
    dict(
