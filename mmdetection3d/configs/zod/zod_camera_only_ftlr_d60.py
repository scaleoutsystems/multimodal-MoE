"""Camera-only BEVFusion for ZOD — Experiment A: gentler fine-tune LR + 60 m depth.

Derived from zod_camera_only.py (baseline camera-only config).
Only two things differ from that baseline:

  1. dbound shortened to [1.0, 60.0, 0.5]
       → tests whether limiting depth range to 60 m sharpens camera BEV
         by discarding the noisy / sparse far-range depth bins.
         Depth bins: (60-1)/0.5 = 118  (vs 178 in baseline).

  2. param_scheduler replaced with a gentler fine-tune / adaptation schedule
       → avoids the aggressive LR spike (lr * 10) of the baseline schedule,
         which can overshoot during partial warm-start adaptation from the
         multi-modal checkpoint.
       Peak LR = 5e-5 (== base lr, never exceeds it).
       Floor LR = 5e-7 (cosine minimum).

Everything else is IDENTICAL to zod_camera_only.py (baseline):
  - Architecture (CameraOnlyBEVFusion, Swin-T, DepthLSSTransform, …)
  - Checkpoint / load_from path
  - Data paths, pipelines, batch sizes
  - splat_radius = 1  (unchanged in this experiment)
  - Hooks, logging, evaluators, all other settings
"""
...
model = dict(
    ...
    # ── camera encoder (IDENTICAL to baseline) ───────────────────────────
    img_backbone=dict(
    ...
    # ── camera → BEV ─────────────────────────────────────────────────────
    # CHANGE vs baseline: dbound shortened to 60 m (was 90 m).
    # splat_radius = 1 is unchanged in this experiment.
    view_transform=dict(
        ...
        dbound=[1.0, 60.0, 0.5],   # shortened: tests sharper near-range BEV
        downsample=2,
        splat_radius=1,
    ...
    # ── detection head (IDENTICAL to baseline) ───────────────────────────
    bbox_head=dict(
...
# ===== optimizer / scheduler ==================================================
# CHANGE vs baseline: gentler fine-tune schedule.
...
#   Rationale: avoids aggressive overshooting during partial warm-start
#   adaptation from the multi-modal checkpoint.
